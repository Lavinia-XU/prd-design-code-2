...
- 表格区是否包含工具栏、搜索筛选、字段、字段展示形式、状态值范围、排序、分页、行内操作和边界状态说明；筛选搜索方式是否优先采用业务设计Skill或Common Design页面模板明确要求；HTML中筛选区是否展示筛选方式来源、筛选组件类型、一个`IxProSearch`高级搜索组件或多个独立组件组合说明，以及筛选字段表格；HTML中表格字段是否渲染为“字段名称、展示形式、组件名称、说明”的表格，非普通文本列是否标注组件名称。
